...
1. 其他BUG修复：https://github.com/yyf330/gf/issues?q=is%3Aissue+label%3Abug+is%3Aclosed
...
1. 将不常用的`guuid`模块迁移到 github.com/yyf330/guuid 作为社区模块维护，保持`gf`主仓库的轻量级。
...
1. 其他一些修复：https://github.com/yyf330/gf/issues?q=is%3Aissue+label%3Abug
...
本次发布即意味下一版本开发计划的开启，欢迎更多小伙伴参与开源贡献：https://github.com/yyf330/gf/projects/8
...
1. 其他一些修复。
1. 详见：https://github.com/yyf330/gf/issues?q=label%3Abug
...
1. `gdb`
    ...
    - 增加大量单元测试；
1. `gredis`
    - 返回数据类型转换改进：https://github.com/yyf330/gf/issues/415
    - 完善单元测试；
    - 其他改进；
...
## Bug Fix
1. 解决`WebSocket`关闭时的`hijacked`报错问题：https://github.com/yyf330/gf/issues/381
1. 解决静态文件服务时大文件的内存占用问题；
1. 修复前置`Nginx`后默认情况下的`Cookie`域名设置问题；
1. 修复`gconv.Struct`在属性为`[]struct`并且输入属性参数为空时的转换失败问题：https://github.com/yyf330/gf/issues/405
1. 其他一些修复；
